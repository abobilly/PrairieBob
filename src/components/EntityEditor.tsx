...
export function EntityEditor({ entity, onChange }: EntityEditorProps) {
  const handleFieldChange = (field: FieldInstance, value: unknown) => {
    const updatedFields = entity.fieldInstances.map((instance) =>
        ...
        <ScrollArea className="h-full pr-2">
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <div className="text-sm font-mono">{entity.__identifier}</div>
            </div>
            <div className="space-y-2">
              <Label className="text-xs">IID</Label>
              <div className="text-xs text-muted-foreground break-all">{entity.iid}</div>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-2">
                <Label className="text-xs">X</Label>
                <Input
                  value={getNumberValue(entity.px[0])}
                  readOnly
              ...
              <div className="space-y-2">
                ...
                <Input
                  value={getNumberValue(entity.px[1])}
                  readOnly
                  className="h-8 text-sm font-mono"
                />
              </div>
            ...
            <div className="space-y-3">
              ...
              {entity.fieldInstances.length === 0 ? (
                    ...
                    return (
                      <div key={field.defUid} className="space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-[10px] text-muted-foreground">{field.__type}</span>
                        </div>
                        {valuesToRender ? (
                          <div className="space-y-2">
                            {valuesToRender.length === 0 ? (
                              ...
                              valuesToRender.map((entry, index) => (
                                <div key={`${field.defUid}-${index}`} className="rounded-md border border-input p-2">
